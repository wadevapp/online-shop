...
<a href="?page=dashboard" class="visible-xs" style="position: absolute;margin-left: 38%;margin-top:10px;font-size:18px;color: white"><span><b><?php echo strtoupper($_SESSION['settingApp']->Name); ?></b></span></a>
  ...
  <footer class="main-footer">
    ...
    <strong>Copyright &copy; 2017 <a href="#"><?php echo $_SESSION['settingApp']->Name; ?></a>.</strong> All rights
    reserved.
  </footer>
      ...
      $.ajax({
        ...
        data:{ id:$(this).attr("data-id"),actions:'GetProduct' } ,
          success: function(data){                         
              ...
              var arrcolors = data.colors;
              var colors = arrcolors.split(',');
              // $("#colors").val(colors);
              // $("#colors").multiselect("refresh");
              // options = document.querySelectorAll('#colors option');
              $('#passport-or-ic-img1').attr('src', data.image);
              // colors.split(',').forEach(function(v) {
              //   Array.from(options).find(c => c.value == v).selected = true;
              // });              
            //   $.each(arrcolors.split(","), function(i,e){
            //     $("#colors option[value='" + e + "']").prop("selected", true);
            // });
...
<script>    
        ...
        //     event.preventDefault();            
        //     var file_data = $("#passport-or-ic1").prop("files")[0];   // Getting the properties of file from file field
        //       var form_data = new FormData();                  // Creating object of FormData class
        //       form_data.append("PassportOrIcImgUrl", file_data);             // Appending parameter named file with properties of file_field to form_data
        //       // form_data.append("user_id", 123);
        //       // form_data.append("actions", 'InsertProduct');           
        //     // product = $(this).serialize();
        //     var actions = $("input[name='actions']").val();
        //     var posting = $.ajax({
        //         processData: false ,
        //         // contentType : false,
        //         // cache:false,
        //         type: "POST",
        //         url: "ajax/ajax-crud-product.php", 
        //         data:{form_data,actions:actions},
        //         success: function(data){
        //         alert('a');                                    
        //                   $('#datatable-ajax-products').DataTable().ajax.reload();                          
        //                   $('.btn').prop('disabled',false);
